...
function setup() { //只會執行一次
  ...
  background("#ffcad4")
  points = font.textToPoints("pop", -300,80, 200, {
    sampleFactor:0.08
...
function draw() { //畫圖
  ...
  rotate((angle/50)%360) //旋轉角度
    for (let i=0; i<points.length-1; i++) { 
 // text(str(i),points[i].x,points[i].y)
 fill("#a9def9") //圓充滿顏色
 noStroke()//圓不要框
 ellipse(points[i].x+r*sin(angle+i*4),points[i].y+r*sin(angle+i*4),10)
 strokeWeight(2)
 stroke("#720026")//畫線的顏色
line(points[i].x+r*sin(angle+i*1),points[i].y+r*sin(angle+i*1),points[i+1].y) //兩點間畫一條線
 } 
